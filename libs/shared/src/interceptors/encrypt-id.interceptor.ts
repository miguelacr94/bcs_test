...
export class EncryptIdInterceptor implements NestInterceptor {
  ...
  intercept(context: ExecutionContext, next: CallHandler): Observable<any> {
    ...
    if (request) {
      ...
      if (request.query && request.query.id) {
        request.query.id = this.cryptoAdapter.decryptId(request.query.id as string);
      }
    }

    // 2. Encriptar IDs en la respuesta devuelta al frontend
    return next.handle().pipe(
      map((data) => this.encryptObjectIds(data)),
    );
  }
  ...
  private decryptObjectIds(obj: any): any {
    ...
    for (const key of Object.keys(newObj)) {
      if ((key === 'id' || key === '_id' || key === 'applicationId' || key === 'activeApplicationId') && typeof newObj[key] === 'string') {
